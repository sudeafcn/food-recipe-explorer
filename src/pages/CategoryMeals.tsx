...
export default function CategoryMeals() {
  ...
  const { data: meals, isLoading, isError } = useQuery({
    ...
    queryFn: () => recipeService.getMealsByCategory(categoryName as string),
    ...
    <div className="space-y-8 mt-4">
      <div className="flex items-center gap-4 border-b pb-4">
        ...
        <h2 className="text-3xl font-extrabold text-slate-800">
          {categoryName} <span className="text-orange-500 text-xl font-medium">Kategorisi</span>
        </h2>
      </div>
      ...
      {(!meals || meals.length === 0) ? (
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
          ...
          <h3 className="text-3xl font-bold text-slate-700 mb-4">Bu kategoride tarif yok!</h3>
          <p className="text-lg text-slate-500 max-w-md mb-8">
            Görünüşe göre {categoryName} kategorisinde henüz bir tarifimiz bulunmuyor. Farklı kategorileri keşfetmeye devam edebilirsin.
          </p>
          <button 
